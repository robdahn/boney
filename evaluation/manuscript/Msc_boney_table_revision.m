%% Create manuscript-revision table
...
fontsize     = 13;
boneyEvalDir = fileparts(which('Msc_boney_table_revision.m'));


% load correlation data prepared in R
men_corr   = cat_io_csv(fullfile(boneyEvalDir,'men_cor_Boney_revision.csv'));
men_pval   = cat_io_csv(fullfile(boneyEvalDir,'men_pval_Boney_revision.csv'));
women_corr = cat_io_csv(fullfile(boneyEvalDir,'women_cor_Boney_revision.csv'));
women_pval = cat_io_csv(fullfile(boneyEvalDir,'women_pval_Boney_revision.csv'));
both_corr  = cat_io_csv(fullfile(boneyEvalDir,'boney_correlation_revision.csv')); 
both_pval  = cat_io_csv(fullfile(boneyEvalDir,'boney_cor_pval_revision.csv'));
%
...
both_corr = cat_io_strrep(both_corr, ...
    {'v_23300_2_0','v_23226_2_0', 'v_51_2_0', 'v_23099_2_0', 'tismri_volr01', ...
    'tismri_volr02', 'tismri_volr03', 'tismri_TIV', 'v_894_2_0',...
    'v_914_2_0', 'r_alcohol', 'v_23239_2_0', 'sROI_bonecortex03',...
    ...
    'v_50_2_0','v_21001_2_0', 'v_22407_2_0', 'v_22408_2_0','v_874_2_0', ...
    'v_20162_2_0'},...
    ...
    'IAP', 'vol-based proxy BMD', 'global bone thickness', ...
    ...
    'height','BMI','VAT', 'ASAT', 'walk','smoking'});
warning on
...
%%
use_vals   = { 
  [28:30 32 43 47 48 44:46 50:53 2:6]  [ 11 8 21 42 35 28:30 43 3 4 5 6 ] 'bone & fat' [2 3 5 8 9 10]  [4 10 14 15 16];   % bone [BMD, brain, age]
  [28:30 32 43 47 48 44:46 50:53 2:6]  [ 28 18 8 19:22 26 27 23:24  42 33:36 40:41 37 38 2:6 ] 'bone & fat' [1 3 4 11 20 21 22]  [4 10 14 15 16]; %supplement figure
  %[7 4 5 6 39 41:44 2 15 12 13 14]  [ 29 32 7 6 5 4 15 12 13 14  ] 'fat' [6 7 ]  [5 9 10 11];%poster OHBM figure
};
...
for uvi = 1:size(use_vals,1)
  ...
  %%
  print(gcf,fullfile(sprintf('Msc_2024_table%02d.png', uvi )),'-dpng','-r1200'); 
